...
## tests/standards/test_standard_10_strict_policy_base_drift.py (5건)
...
- 모든 sentinel 은 CI guard `check_standard_10.py` 의 reusable API 를
  import 하여 결정적으로 검증.
- 임의 fixture 사용 — 측정값 변동 없음 (단위 로직 검증 한정).
- 신규 sentinel 5건 — 회귀 카운트에 가산.

## 측정 결과

- tests/standards/test_standard_10_strict_policy_base_drift.py : 5 passed
- tests/standards/ 누적 : 19 passed (PR #728 정착 14 + Standard 10 신규 5)
