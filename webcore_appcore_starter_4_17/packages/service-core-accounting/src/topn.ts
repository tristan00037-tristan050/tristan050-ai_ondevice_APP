...
export type Candidate = { code: string; score: number; reasons: string[] };
...
function amountBucket(amtKRW: number): 'micro' | 'small' | 'mid' | 'large' | 'xl' {
  if (amtKRW <= 5_000) return 'micro';
  if (amtKRW <= 30_000) return 'small';
  if (amtKRW <= 150_000) return 'mid';
  if (amtKRW <= 1_000_000) return 'large';
  return 'xl';
}
...
// 금액 버킷 기반 백업 후보
function bucketFallbacks(amtKRW: number): Candidate[] {
...
// 매핑 함수가 있으면 존중
function mappedPrimary(desc: string): Candidate[] {
...
// 최종 후보 확장기 (Top-N 풍부화)
export function expandAndRankCandidates(desc: string, amtKRW: number, topN = 8): Candidate[] {
  const cands = [
    ...mappedPrimary(desc),
