...
import { applyMapping, type MappingRule } from './rules.js';

/**
 * 기본 계정 매핑 규칙 (한국 회계 기준)
 */
const DEFAULT_ACCOUNT_MAPPINGS: MappingRule[] = [
  // 자산 계정 (1xxx)
  { from: '현금', to: '1010' },
  { from: '예금', to: '1010' },
  { from: '보통예금', to: '1010' },
 ...
 */
export function mapAccount(description: string): string | null {
 ...
 * 계정 타입 판별 (차변/대변 결정)
 */
export function getAccountType(accountCode: string): 'asset' | 'liability' | 'equity' | 'revenue' | 'expense' | 'unknown' {
