...
app.get('/health', async (req, res) => {
    try {
        ...
        await notifyDatabaseConnectionFailure(error);
        res.status(503).json({
            status: 'degraded',
            service: 'collector',
...
app.get('/timeline', requireTenantAuth, async (req, res) => {
    try {
        const windowH = parseInt(req.query.window_h) || 24;
...
app.get('/admin/audit/logs', requireTenantAuth, async (req, res) => {
    try {
        ...
        const startTime = req.query.start_time ? parseInt(req.query.start_time) : undefined;
        const endTime = req.query.end_time ? parseInt(req.query.end_time) : undefined;
