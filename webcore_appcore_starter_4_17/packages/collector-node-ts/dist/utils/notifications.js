 ...
 * PagerDuty 알림 전송
 */
async function sendPagerDutyNotification(notification) {
 ...
 * 성능 저하 알림
 */
export async function notifyPerformanceDegradation(metric, threshold, currentValue) {
    await sendNotification({
        level: 'warning',
 ...
 * 용량 임계값 알림
 */
export async function notifyCapacityThreshold(resource, usage, threshold) {
 ...
 * 데이터베이스 연결 실패 알림
 */
export async function notifyDatabaseConnectionFailure(error) {
    await sendNotification({
        level: 'critical',
 ...
 * Rate Limit 초과 알림
 */
export async function notifyRateLimitExceeded(tenantId, endpoint, limit) {
    await sendNotification({
        level: 'warning',
