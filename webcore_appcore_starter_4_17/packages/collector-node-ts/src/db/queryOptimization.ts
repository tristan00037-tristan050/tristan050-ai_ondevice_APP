 ...
 * 쿼리 실행 계획 분석
 */
export async function explainQuery(sql: string, params?: unknown[]): Promise<unknown> {
 ...
 * 인덱스 사용 여부 확인
 */
export async function checkIndexUsage(tableName: string, columnName: string): Promise<boolean> {
...
export async function getSlowQueries(limit: number = 10): Promise<Array<{
  ...
  try {
    ...
    return result.rows.map((row: pg.QueryResultRow) => {
      const r = row as {
        query: string;
        calls: number;
        total_time: number;
 ...
 * 인덱스 최적화 제안
 */
export async function suggestIndexes(tableName: string): Promise<Array<{
  column: string;
  reason: string;
}>> {
  const suggestions: Array<{ column: string; reason: string }> = [];
