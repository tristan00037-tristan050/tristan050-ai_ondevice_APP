 ...
 * 리포트 조회
 */
export async function getReport(id: string, tenantId: string): Promise<Report | null> {
  const result = await query<{
    ...
    `SELECT id, tenant_id, report_data, markdown, created_at, updated_at
     FROM reports
...
export async function listReports(
  ...
  const reports = result.rows.map((row: {
    ...
    const reportData = row.report_data as any;
    
    // severity 추출 (가장 높은 severity)
    let maxSeverity: 'info' | 'warn' | 'block' | undefined;
    if (reportData?.policy?.evaluations) {
      const severities = reportData.policy.evaluations
        .map((e: any) => e.severity)
        .filter((s: string) => s === 'info' || s === 'warn' || s === 'block');
      if (severities.includes('block')) maxSeverity = 'block';
      else if (severities.includes('warn')) maxSeverity = 'warn';
      else if (severities.includes('info')) maxSeverity = 'info';
    }
