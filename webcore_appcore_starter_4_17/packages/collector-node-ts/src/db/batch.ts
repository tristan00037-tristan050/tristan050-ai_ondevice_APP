 ...
 * 배치 리포트 저장 (트랜잭션 사용)
 */
export async function batchSaveReports(reports: Report[]): Promise<void> {
 ...
 * 시간 버킷별로 그룹화하여 집계
 */
export async function batchAggregateTimeline(
  tenantId: string,
  startTime: number,
  endTime: number,
  bucketSizeMs: number = 3600000 // 1시간
): Promise<Array<{ time: number; info: number; warn: number; block: number }>> {
  ...
  const result = await query<{
    ...
    [tenantId, startTime, bucketSizeMs, endTime]
  );
