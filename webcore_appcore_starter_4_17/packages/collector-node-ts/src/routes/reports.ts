...
import { observeResponseTime, incrementError, incrementSignRequest, incrementBundleDownload } from '../metrics/prometheus.js';
import { getCachedReportsList, setCachedReportsList, invalidateOnReportSave } from '../cache/reports.js';
...
router.get('/', requireTenantAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const policyVersion = req.query.policy_version as string | undefined;
    ...
    const { reports: tenantReports, totalCount } = await reportsDb.listReports(
      tenantId,
      {
        severity: severity as 'info' | 'warn' | 'block' | undefined,
        policyVersion,
        since,
      },
      { page, limit }
...
interface SignHistory {
  reportId: string;
  tenantId: string;
  requestedBy: string; // tenantId 또는 사용자 식별자
  token: string;
  issuedAt: number; // iat
...
const signHistory: SignHistory[] = [];

/**
 * 리포트 서명 요청 (멱등성 보장)
 * POST /reports/:id/sign
 */
router.post('/:id/sign', requireTenantAuth, async (req: Request, res: Response) => {
  const startTime = Date.now();
  try {
    const tenantId = req.tenantId;
 ...
 */
router.get('/:id/sign-history', requireTenantAuth, async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
 ...
 */
router.get('/:id/bundle-meta', requireTenantAuth, async (req: Request, res: Response) => {
  try {
    const tenantId = req.tenantId;
 ...
 */
router.get('/:id/bundle.zip', verifySignToken, async (req: Request, res: Response) => {
  const startTime = Date.now();
  try {
    const tenantId = req.tenantId;
