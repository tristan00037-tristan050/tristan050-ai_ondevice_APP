...
class RateLimitStore {
   ...
   * 요청 카운트 증가
   */
  increment(key: string, windowMs: number): { count: number; resetAt: number; remaining: number } {
    const now = Date.now();
 ...
 * Rate Limit 키 생성
 */
function getRateLimitKey(req: Request, type: 'tenant' | 'ip' | 'token'): string {
  switch (type) {
    case 'tenant':
    ...
    if (count > config.maxRequests) {
      ...
      import('../utils/notifications.js').then(({ notifyRateLimitExceeded }) => {
        notifyRateLimitExceeded(
          (req as Request & { tenantId?: string }).tenantId || 'unknown',
          req.path,
          config.maxRequests
        ).catch(console.error);
      }).catch(() => {
