...
# 증빙 로그 시작
{
  ...
  # 2) build_info.json 확인
  echo "=== Step 2: build_info.json ==="
  ...
  # 3) healthz 헤더 확인 (메타만, 본문 덤프 금지)
  echo "=== Step 3: healthz Headers ==="
  curl -iS --max-time 3 http://127.0.0.1:8081/healthz 2>&1 | grep -E "HTTP|X-OS-Build" | head -5
  echo ""
  
  # 4) healthz JSON 확인 (메타만)
  echo "=== Step 4: healthz JSON ==="
