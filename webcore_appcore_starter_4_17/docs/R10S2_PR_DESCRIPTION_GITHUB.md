...
이번 PR은 **AI 온디바이스 Enterprise OS 레이어**의 LLM 서비스 표준화를 완료합니다. DomainLLMService를 공통 패키지로 승격하여 다도메인 확장 기반을 마련하고, LLM Usage Audit에 eventType을 도입해 KPI 측정이 가능하도록 했습니다. Remote LLM Gateway 계약(501 stub)과 3단계 후처리 Hook 구조를 확립하여 Playbook 원칙(온디바이스 우선, 게이트웨이 경계, Mock/Live 쌍 유지)을 준수합니다.
