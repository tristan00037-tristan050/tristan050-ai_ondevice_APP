...
### mock + rule
- **엔진**: LocalRuleEngineV1Adapter
- **응답 텍스트**:
  ```
  응답 추천:
  [Rule] "Mock CS Ticket 1" 문의에 대한 규칙 기반 Mock 응답입니다.
  ```
- **지연 시간**: 즉시 (지연 없음)
- **네트워크**: 0건
...
### live + local-llm
- **엔진**: LocalLLMEngineV1 (동일 Stub)
- **응답 텍스트**: mock + local-llm과 동일
- **지연 시간**: 약 1.8초 후 표시
- **네트워크**: BFF에서 티켓 리스트 로드 시에만 요청 발생 (추천 요청은 네트워크 없음)
