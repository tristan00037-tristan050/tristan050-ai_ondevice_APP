...
DEADLINE_PATTERNS: List[re.Pattern[str]] = [
    ...
    re.compile(r"이번\s*주\s*[월화수목금토일]요일\s*(?:에는?|까지)?"),            # 이번 주 금요일에/까지
    ...
    re.compile(r"다음\s*주\s*(?:(?:월|화|수|목|금|토|일)요일\s*)?까지?"),        # 다음 주까지 / 다음 주 월요일까지
...
_SENTENCE_TYPE_RES: dict[SentenceType, re.Pattern[str]] = {
    ...
    SentenceType.NEGATIVE: re.compile(
        ...
        r"|되지\s*않|안\s*됩니다"
        r"|없어서|없으니|취소(?:되었|됩니다|합니다)?|어렵습니다"
        r"|보류(?:되었|합니다)?|연기(?:되었|합니다)?|불가합니다?"
        r"|못\s*합니다|진행하지)"
    ),
}
