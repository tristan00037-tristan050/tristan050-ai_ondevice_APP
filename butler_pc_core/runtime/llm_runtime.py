...
def _completion_text(output: Any) -> str:
    """Normalize llama completion output without falling back to str(dict/generator)."""
    if isinstance(output, str):
        return output
    if not isinstance(output, dict):
        raise TypeError("MODEL_RESPONSE_UNSUPPORTED")
    choices = output.get("choices")
    if not isinstance(choices, list) or not choices:
        raise TypeError("MODEL_RESPONSE_CHOICES_MISSING")
    first = choices[0]
    if not isinstance(first, dict):
        raise TypeError("MODEL_RESPONSE_CHOICE_INVALID")
    text = first.get("text")
    ...
    raise TypeError("MODEL_RESPONSE_TEXT_MISSING")


def _completion_call_kwargs(
    *,
    max_tokens: int,
    temperature: float,
    ...
    if grammar is not None:
        ...
        # Structured JSON path only. Do not inject this into grammar=None legacy calls.
        call_kwargs["temperature"] = 0.1
        call_kwargs.setdefault("top_p", 0.95)
    return call_kwargs
...
class LlmRuntime:
    ...
    def generate(
        ...
        call_kwargs = _completion_call_kwargs(
            max_tokens=max_tokens,
            temperature=temperature,
        ...
        return _strip_residual_stop_tokens(_completion_text(output))
        ...
        with self._lock:
            for chunk in self._llm(prompt, **call_kwargs):
                if cancel_event.is_set():
                    break
                tokens.append(_completion_text(chunk))
