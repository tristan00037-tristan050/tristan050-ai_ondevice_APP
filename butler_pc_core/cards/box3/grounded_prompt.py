...
OUTPUT_FORMAT_STRICT (PR #785 v1.2.2 추가, 2026-06-05; PR #786 정정 보강):
...
각 섹션 라벨(제목/배경/핵심 내용/근거/확인 필요/최종 문안) 뒤에는 반드시 완성된 한국어
문장을 1개 이상 작성하세요. 빈 라벨만 반복해 줄을 채우지 마세요. 같은 라벨 패턴을
2번 이상 반복하면 안 됩니다. 근거가 없으면 그 섹션에 "{ABSTAINED_SLOT_TEXT}" 한
줄만 적고 다음 섹션으로 넘어가세요. 임의로 [공문]·[보고물]·[결론]·[추가사항]·
[참조사항]·[후속조치] 같은 다른 라벨 셋을 새로 만들지 마세요.
