...
V9_1_SYSTEM_PROMPT = (
    ...
    "8. 날짜·금액·수량은 [근거 카드]의 항목명 그대로만 씁니다. 예: 근거에 '〈항목명〉: 〈값〉'이 있으면 항목명 '〈항목명〉'을 그대로 쓰고 다른 항목명으로 바꾸지 마십시오.\n"
...
# V9_1_OUTPUT_SKELETON 의 순수 플레이스홀더/지시 문구 — 실제 grounded 초안이라면 [근거 카드]의
# 항목명·값으로 치환되어 사라져야 한다. 이 문구가 초안에 그대로 남아 있으면 모델이 골격을
# 실제 내용으로 착각해 에코한 것(정상 근거 문장에는 등장하지 않는다). "근거 범위에서 확인" 등
# 정당한 마무리 문구는 오탐 방지를 위해 제외한다.
OUTPUT_SKELETON_ECHO_MARKERS: tuple[str, ...] = (
    "주제 정보 요약",
    "복사할 값입니다",
    "항목명은/는",
    "항목명과 복사할 값으로 바꿉니다",
...
# build_citation_card 가 만드는 [근거 카드] 스캐폴딩 메타 문구 — 실제 grounded 초안이라면
# 절대 등장하지 않아야 한다. 모델이 이 메타를 실제 내용으로 착각해 에코하면 유출이다.
# 콜론 포함 정확형과 관측된 축약형만 마커로 둔다: SYSTEM 규칙3의
# "…바꿔쓰기 금지."(마침표형)와의 오탐 방지.
EVIDENCE_CARD_META_ECHO_MARKERS: tuple[str, ...] = (
    "바꿔쓰기 금지: 예",
    "바꿔쓰기: 예",
    "보존할 문구:",
    "반드시 복사할 값:",
