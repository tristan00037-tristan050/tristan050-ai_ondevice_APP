...
### 3-5. 로컬 실행 및 Live QA 표준 SOP

- **로컬 실행/Live QA는 스크립트만 사용한다. 수동 kill/export 금지.**
  - BFF 실행: `./scripts/dev_bff.sh restart`
  - Web 실행: `./scripts/dev_web.sh`
  - 상태 점검: `./scripts/dev_check.sh`
  - 정책 체크: `npm run policy:check`
- **dev 환경에서만 CORS preflight header reflect를 허용한다**
  - `NODE_ENV !== production` + `localhost/127.0.0.1 origin` 제한
