...
def test_reads_bundled_stamp(tmp_path, monkeypatch):
    ...
    stamp.write_text(json.dumps({
        ...
        "git_describe": "v0.9.0", "builder": "build_safe_source_archive.py",
    }), encoding="utf-8")
    ...
    assert mod.build_tree_oid() == "e" * 40
...
def test_missing_stamp_is_unknown_not_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(mod, "_git_tree", lambda: "unknown")
    mod.build_info.cache_clear()
    info = mod.build_info()
    assert info["build_base_commit_oid"] == "unknown"
    assert info["source"] == "runtime_fallback"
    assert set(info) >= {"app", "build_base_commit_oid", "build_tree_oid", "build_timestamp_utc", "app_version"}
...
def test_incomplete_or_unbound_stamp_falls_back(tmp_path, monkeypatch):
    mod = _fresh()
    stamp = tmp_path / "BUILD_INFO.json"
    stamp.write_text(json.dumps({"app": "Butler", "build_tree_oid": "unknown"}), encoding="utf-8")
    monkeypatch.setattr(mod, "_stamp_path", lambda: stamp)
    monkeypatch.setattr(mod, "_git_head", lambda: "a" * 40)
    monkeypatch.setattr(mod, "_git_tree", lambda: "b" * 40)
    mod.build_info.cache_clear()
    assert mod.build_info()["source"] == "runtime_fallback"
    assert mod.build_tree_oid() == "b" * 40


def test_structurally_complete_but_invalid_stamp_falls_back(tmp_path, monkeypatch):
    mod = _fresh()
    stamp = tmp_path / "BUILD_INFO.json"
    stamp.write_text(json.dumps({
        "app": "Butler", "build_base_commit_oid": "a" * 40, "build_tree_oid": "b" * 40,
        "build_timestamp_utc": "not-a-timestamp", "app_version": "0.9.0\nforged",
        "git_describe": "v0.9.0", "builder": "builder.py",
    }), encoding="utf-8")
    monkeypatch.setattr(mod, "_stamp_path", lambda: stamp)
    monkeypatch.setattr(mod, "_git_head", lambda: "c" * 40)
    monkeypatch.setattr(mod, "_git_tree", lambda: "d" * 40)
    mod.build_info.cache_clear()
    assert mod.build_info()["source"] == "runtime_fallback"
    assert mod.build_commit_oid() == "c" * 40
