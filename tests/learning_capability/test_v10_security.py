...
def test_strict_json_rejects_non_bytes_and_numeric_overflow():
    with pytest.raises(StrictJsonError, match="JSON_BYTES_REQUIRED"):
        load_strict_bytes('{"a":1}')  # type: ignore[arg-type]
    with pytest.raises(StrictJsonError, match="JSON_NON_FINITE_FORBIDDEN"):
        load_strict_bytes(b'{"a":1e9999}', allow_float=True)
    with pytest.raises(StrictJsonError, match="JSON_INTEGER_TOO_LARGE"):
        dump_canonical_bytes({"a": 12_345_678_901_234_567})
