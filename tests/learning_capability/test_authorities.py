...
class _DriftingBindings:
    ...
    def snapshot_for(self, _key, _digests) -> tuple[bool, str]:
        self.calls += 1
        return False, f"{self.calls:064x}"


class _StaticAuthority:
    def __init__(self, key: str, revision: str) -> None:
        self.key = key
...
def test_binding_revision_mismatch_blocks_the_whole_snapshot(
    tmp_path,
    monkeypatch,
):
    policy_store = PolicyStore(root=tmp_path / "policy")
    _install_policy(policy_store, monkeypatch)
    policy_authority = CompanyPolicyAuthority(
        policy_store,
        _DriftingBindings(),
    )
    ...
    try:
        ...
        raise AssertionError("drifting binding unexpectedly produced snapshot")
    assert not (tmp_path / "generation.json").exists()


class _FailingBindings:
    def record(self, *_args, **_kwargs) -> None:
        raise ConsumerBindingError("BINDING_SAVE_FAILED")


def test_binding_save_failure_preserves_product_response_without_in_use_claim(
    tmp_path,
    monkeypatch,
):
    policy_store = PolicyStore(root=tmp_path / "policy")
    _install_policy(policy_store, monkeypatch)
    proof_store = ConsumerBindingStore(tmp_path / "bindings.json")
    authority = CompanyPolicyAuthority(policy_store, proof_store)
    monkeypatch.setattr(
        policy_middleware,
        "default_consumer_binding_store",
        lambda: _FailingBindings(),
    )
    app = FastAPI()
    add_policy_gate_middleware(app, policy_store=policy_store)
