...
def test_score_tie_keyword_wins_over_vendor_only():
    ...
    재현 경로:
    - 통신비(ACCOUNTS index 7): KT + SKT vendor 각 1회 매칭 → raw=1.0, conf=0.30
    - 임차료(ACCOUNTS index 10): "임차료" keyword 매칭 → raw=1.0, conf≥0.50
    - 수정 전: 통신비가 먼저 잠겨 conf<0.50 → 미분류 반환 (버그)
    - 수정 후: 동점에서 conf 높은 임차료 선택 → 임차료, conf≥0.50
    """
    from butler_pc_core.accounting.account_dict import match_account

    name, conf = match_account("KT SKT 임차료 납부", "")
    assert name == "임차료", (
        f"동점 처리 실패 — 기대: '임차료', 실제: '{name}' "
...
def test_score_tie_reflected_in_classify_df():
    ...
    df = pd.DataFrame([
        {"적요": "KT SKT 임차료 납부", "거래처": ""},
        {"적요": "KT 보험료납부 처리", "거래처": "SKT"},
    ])
    result = classify_df(df)

    assert result.iloc[0]["분류과목"] == "임차료", (
