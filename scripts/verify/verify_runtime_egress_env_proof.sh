...
cleanup(){
  ...
  # Template verification must pass; proof is optional (generated by ops scripts)
  if [[ "${RUNTIME_EGRESS_ENV_TEMPLATE_OK}" == "1" ]]; then
    exit 0
  fi
  exit 1
}
...
# Note: Proof files are generated by ops scripts in actual deployment environments
# If proofs don't exist yet, template verification still passes
...
if [[ -s "${PROOF1}" ]] && [[ -s "${PROOF2}" ]]; then
  ...
  # Proof files not yet generated (will be created by ops scripts in deployment)
  # Template verification passes, but proof verification is pending
  echo "SKIP: Proof files not yet generated (will be created by ops scripts)"
  RUNTIME_EGRESS_ENV_PROOF_OK=0
fi
