...
cleanup() {
  ...
  if [[ "$AI_VARIANCE_MEASUREMENTS_SOURCE_OK" == "1" ]] && \
     [[ "$AI_VARIANCE_OK" == "1" ]] && \
...
measurements_file="scripts/ai/fixtures/variance_outlier_measurements_v1.json"
test -f "$measurements_file" || { echo "BLOCK: missing measurements file: $measurements_file"; exit 1; }

# 3) Run verification with file input
OUT="$(node scripts/ai/verify_variance_outlier_v1.cjs --measurements_json "$measurements_file" 2>&1)" || { echo "BLOCK: verification failed"; echo "$OUT"; exit 1; }

echo "$OUT" | grep -q '^AI_VARIANCE_MEASUREMENTS_SOURCE_OK=1$' || exit 1
