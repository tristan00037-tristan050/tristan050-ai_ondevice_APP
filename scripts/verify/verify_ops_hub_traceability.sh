...
echo "$OUT" | rg -n 'JOIN_LINE request_id=.* joinable=true' >/dev/null || { echo "FAIL: missing join line"; exit 1; }
echo "$OUT" | rg -n 'joinable=true_count=[1-9]' >/dev/null || { echo "FAIL: joinable true count < 1"; exit 1; }
OPS_HUB_TRACEABILITY_REPORT_OK=1

# 2) no-raw(대표 금지 패턴)
if echo "$OUT" | rg -n '(raw_text|prompt|messages|document_body|content|BEGIN .* PRIVATE KEY|_TOKEN=|_PASSWORD=|DATABASE_URL=)' ; then
