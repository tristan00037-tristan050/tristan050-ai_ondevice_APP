...
node "$PKG/src/server.mjs" --smoke >/tmp/runtime_shadow.out 2>&1 &
RUNTIME_PID=$!

# Wait for server
for i in $(seq 1 50); do
  if grep -q "RUNTIME_LISTENING=1" /tmp/runtime_shadow.out; then break; fi
  sleep 0.1
done
...
export BUTLER_RUNTIME_URL="http://127.0.0.1:8091"
...
# Start runtime
cd "$ROOT"
node "$PKG/src/server.mjs" --smoke >/tmp/runtime_proof.out 2>&1 &
RUNTIME_PID=$!

# Wait for runtime to be ready
for i in $(seq 1 50); do
  if grep -q "RUNTIME_LISTENING=1" /tmp/runtime_proof.out 2>/dev/null; then break; fi
  sleep 0.1
done

# Start BFF
cd webcore_appcore_starter_4_17/packages/bff-accounting
node dist/index.js >/tmp/bff_off.out 2>&1 &
BFF_OFF_PID=$!

# Wait for BFF to be ready (check for listening port or health endpoint)
for i in $(seq 1 100); do
  if curl -sS "http://127.0.0.1:8081/healthz" >/dev/null 2>&1; then break; fi
  sleep 0.1
done

# Verify BFF is actually running
if ! curl -sS "http://127.0.0.1:8081/healthz" >/dev/null 2>&1; then
  echo "BLOCK: BFF server failed to start"
  echo "BFF logs:"
  cat /tmp/bff_off.out 2>&1 || true
...
RESPONSE_OFF="$(curl -sS -D /tmp/bff_off_headers.txt -X POST "http://127.0.0.1:8081/v1/os/algo/three-blocks" \
  -H "Content-Type: application/json" \
...
node dist/index.js >/tmp/bff_on.out 2>&1 &
BFF_ON_PID=$!
...
# Call with shadow ON
RESPONSE_ON="$(curl -sS -D /tmp/bff_on_headers.txt -X POST "http://127.0.0.1:8081/v1/os/algo/three-blocks" \
  -H "Content-Type: application/json" \
  -H "X-Api-Key: dev-key:operator" \
  -H "X-Tenant: default" \
  -H "X-User-Id: test-user" \
...
HEADER_OFF_LATENCY="$(grep -i "^x-os-algo-latency-ms" /tmp/bff_off_headers.txt | head -1 || echo "")"
HEADER_ON_LATENCY="$(grep -i "^x-os-algo-latency-ms" /tmp/bff_on_headers.txt | head -1 || echo "")"
...
HEADER_OFF_SHA="$(grep -i "^x-os-algo-manifest-sha256" /tmp/bff_off_headers.txt | head -1 || echo "")"
HEADER_ON_SHA="$(grep -i "^x-os-algo-manifest-sha256" /tmp/bff_on_headers.txt | head -1 || echo "")"
