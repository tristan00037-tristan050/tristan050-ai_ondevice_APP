...
cleanup() {
  ...
  if [[ "$HIGH_RISK_BLOCK_WITHOUT_APPROVAL_OK" == "1" ]] && \
     ...
     [[ "$HIGH_RISK_TAINT_HIGH_SCOPE_ENFORCED_OK" == "1" ]]; then
    exit 0
  fi
  exit 1
}
...
echo "$OUT" | grep -nE '^HIGH_RISK_TAINT_STRING_BLOCK_OK=1$' >/dev/null || exit 1
HIGH_RISK_TAINT_STRING_BLOCK_OK=1

echo "$OUT" | grep -nE '^HIGH_RISK_TAINT_HIGH_SCOPE_ENFORCED_OK=1$' >/dev/null || exit 1
