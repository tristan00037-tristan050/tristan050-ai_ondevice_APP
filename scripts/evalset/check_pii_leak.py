...
P2 정정 (2026-05-13, PR #702 리뷰):
  fail-closed 원칙. JSON parse 오류도 leak 과 별도로 ok=False 로 차단.
  손상된 JSON 줄은 leak 검출이 불가능하므로 통과시키면 안 됨.

PASS: exit 0, 보고서 ok=true + leak_count=0 + parse_error_count=0
FAIL: exit 1, fail_class=PII_LEAK 또는 JSON_PARSE_ERROR
"""
...
def main() -> int:
    ...
    with in_path.open(encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            try:
                ...
                # P2 정정: parse error 는 별도 분리 + fail-closed
    ...
    ok = (leak_total == 0) and (parse_total == 0)
    if not ok:
        # P2 정정: parse error 우선 보고 (검사 자체가 불가능하므로 더 심각)
        if parse_total > 0:
