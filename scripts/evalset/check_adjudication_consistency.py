#!/usr/bin/env python3
"""check_adjudication_consistency.py — 단계 6.5.5 Day 4~5 CI Gate G17~G21.

알고리즘 팀 Day 4~5 차단 기준을 단일 스크립트로 통합 검사. fail-closed.
  ...
  G19 APPROVED_WITHOUT_DISAGREEMENT_RESOLUTION
        annotator_a/b 불일치 sample 인데 label_status ∈ {adjudicated, gold_v1}
        인 경우 final_gold.disagreement_resolution 누락
  G20 AUTO_APPLY_REASONING_MISSING
        top-level auto_apply_allowed=true OR final_gold.auto_apply_allowed=true
        인데 auto_apply_reasoning 누락/짧음(< 10자).
...
def _validate_item(item: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # G20 — top-level OR final_gold 둘 다 검사 (P2-B 정착)
    fg       = item.get("final_gold") or {}
    ...
    # G21 — top vs final_gold auto_apply 정합성 (P2-C 정착)
