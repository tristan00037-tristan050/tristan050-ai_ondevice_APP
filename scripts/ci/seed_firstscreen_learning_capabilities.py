...
def _seed() -> None:
    ...
    print("FIRSTSCREEN_LEARNING_SEED=PASS")


def _corrupt_policy() -> None:
    app_data = _approved_app_data()
    index = app_data / "company-policy" / "active_policy_ref.json"
    if not index.is_file():
        raise RuntimeError("E2E_POLICY_INDEX_MISSING")
    index.write_text("{", encoding="utf-8")
    print("FIRSTSCREEN_LEARNING_CORRUPTION=PASS")
