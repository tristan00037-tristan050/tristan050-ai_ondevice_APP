...
def d2_classify(text: str, pd_orig: str,
                orig_actionable: bool | None = None) -> Tuple[str, bool]:
    """D2-A~E targeted 패치 — (deadline_type, patched_actionable).

    우선순위: D2-C/D2-D/D2-E (non-actionable) > D2-A (HARD) > D2-B (SOFT).

    Codex P1 정정: orig_actionable 는 pre-patch deadline_is_actionable.
      D2-A~E 미매칭 시 모델 원본 actionable 을 보존한다 (D-2 는 매칭된
      영역만 actionable 을 override). None 이면 pd_orig 에서 파생 (sentinel
      2-arg 호출 하위호환). D2-A~E 패턴/로직 자체는 불변.
    """
    def _fallback_actionable() -> bool:
        if orig_actionable is not None:
            return bool(orig_actionable)
        return pd_orig in {"HARD", "SOFT"}

    if not text:
        return pd_orig, _fallback_actionable()
...
def measure_deadline(items: List[Dict], preds: List[Dict],
    ...
    """mode = 'baseline_d1' (pre-patch actionable) | 'd2_targeted' (D-2 patched).

    Codex P1 정정: false_deadline 은 mode 별 actionable 기준으로 산출.
      baseline_d1  → pre-patch deadline_is_actionable (PR #724 기준 재현)
      d2_targeted  → d2_classify 가 반환한 patched_actionable
    이전 코드는 두 mode 모두 pre-patch 필드를 사용해 D-2 보정 actionable
    (D2-C/D/E non-actionable, D2-A/B actionable override) 이 false_deadline_rate
    에 반영되지 않았다.
    """
    if mode not in {"baseline_d1", "d2_targeted"}:
        raise ValueError(f"Unknown measure_deadline mode: {mode}")
    ...
    for sid, gold in items_by_id.items():
        ...
        orig_actionable = bool(rec["pred"].get("deadline_is_actionable", False))
        if mode == "baseline_d1":
            pd = d1_classify(text, pd_orig)
            patched_actionable = orig_actionable
        else:   # d2_targeted
            pd, patched_actionable = d2_classify(
                text, d1_classify(text, pd_orig), orig_actionable)
        ...
        if gd in {"INQUIRY", "URGENCY", "CONDITION"}:
            ...
            if pd == gd:
                inq_preserved += 1
        if patched_actionable and gd in {"NONE", "INQUIRY", "URGENCY", "CONDITION"}:
            false_deadline += 1
    ...
    return {
        ...
        "mode":                        mode,
        "computed_from_d2_actionable": mode == "d2_targeted",
    }
...
def main() -> int:
    ...
    full_eval = {
        **_meta(),
        ...
        # safety (Codex P1 정정: false_deadline 은 D-2 patched actionable 기준)
        "false_deadline_rate":          d2["false_deadline_rate"],
        "false_deadline_count":         d2["false_deadline_count"],
        "false_deadline_rate_baseline": baseline["false_deadline_rate"],
        "false_deadline_mode":          d2["mode"],
        "computed_from_d2_actionable":  d2["computed_from_d2_actionable"],
    ...
    (OUT / "external_beta_readiness_update.md").write_text("\n".join([
        ...
        f"- false_deadline_rate: {d2['false_deadline_rate']} (safety gate ≤ 0.02 — "
        f"{'충족' if d2['false_deadline_rate'] <= 0.02 else '미달'}; "
        f"computed_from_d2_actionable=true)",
        "",
        ...
        f"- deadline_f1 0.8702 정합 유지 (D-2 mode 측정 — 정정 영향은 산식만)",
        "",
    ...
    (OUT / "summary.md").write_text("\n".join([
        ...
        f"- false_deadline_rate: baseline {baseline['false_deadline_rate']} → "
        f"d2_targeted {d2['false_deadline_rate']} "
        f"(count {baseline['false_deadline_count']} → {d2['false_deadline_count']})",
