...
grep -R "PASS_BOX3_REAL_LOCAL_AFTER_HUMAN_APPROVAL.*READY" -n . --exclude-dir=.git && fail "runtime PASS overclaim" || true
grep -R "Co-Authored-By\|Generated by Claude\|🤖" -n . --exclude-dir=.git && fail "AI footer detected" || true
pass CLAIM_AND_FOOTER_ZERO

find . -type d -name __pycache__ -print -quit | grep -q . && fail "__pycache__ present" || true
git ls-files 2>/dev/null | grep -E '\.(gguf|safetensors|bin)$' && fail "tracked binary/model artifact present" || true
pass TRACKED_MODEL_BINARY_ZERO

grep -R "helper3.*safetensors\|helper5.*safetensors" -n scripts butler_pc_core butler-desktop --exclude-dir=.git && fail "helper3/5 safetensors reference detected" || true
