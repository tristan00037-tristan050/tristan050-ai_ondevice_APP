...
if _FASTAPI_AVAILABLE:
    ...
    async def _capability_token_middleware(request, call_next):
        ...
        if local_authenticated_request or request.method in ("POST", "PUT", "PATCH", "DELETE"):
            ...
            # Transport authentication is not human identity or an RBAC role.
            # Legacy non-Box5 handlers still require a stable audit subject, so
            # expose an explicitly transport-scoped pseudonym only.
            request.state.capability_actor_id = "local-ipc:" + session.session_digest[:32]
    ...
    # Box5 must authenticate with the exact transport credential generated by
    # this sidecar.  A route-local token manager makes isolated tests pass but
    # leaves the assembled product path permanently unauthenticated.
    install_product_accounting_dependencies(app, _TOKEN_MANAGER)
