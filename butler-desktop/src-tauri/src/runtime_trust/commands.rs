...
use super::keychain;
use super::state::{canonical, sha256, validate_state, NativeTrustReceipt, TrustedState, VerifyAndCommitRequest, RECEIPT_SCHEMA, STATE_SCHEMA};
...
pub fn verify_and_commit_trust_update(
    ...
    if let Some(state) = persisted.as_ref() { validate_state(state).map_err(str::to_owned)?; }
    let in_memory_digest = guard.as_ref().map(|state| canonical(state).map(|raw| sha256(&raw))).transpose().map_err(str::to_owned)?;
    let persisted_digest = persisted.as_ref().map(|state| canonical(state).map(|raw| sha256(&raw))).transpose().map_err(str::to_owned)?;
    if in_memory_digest != persisted_digest { return Err("BLOCK_TRUST_STATE_CAS".into()); }
    ...
    let read_back = keychain::read().map_err(str::to_owned)?.ok_or("BLOCK_TRUST_STATE_CAS")?;
    if read_back != encoded { return Err("BLOCK_TRUST_STATE_CAS".into()); }
