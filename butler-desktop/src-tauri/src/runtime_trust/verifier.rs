...
fn verify_threshold(
    ...
    for envelope in signatures(document)? {
        ...
        let public = URL_SAFE_NO_PAD.decode(string(key, "public_key")?).map_err(|_| error)?;
        let signature = URL_SAFE_NO_PAD.decode(string(envelope, "signature")?).map_err(|_| error)?;
...
fn revoked_keys(revocations: &Value) -> Result<BTreeSet<String>, &'static str> {
    revocations.get("revoked_key_ids").and_then(Value::as_array)
        .ok_or("BLOCK_NATIVE_TRUST_VERIFICATION")?.iter()
        .map(|item| item.as_str().map(str::to_owned).ok_or("BLOCK_NATIVE_TRUST_VERIFICATION"))
        .collect()
}

pub fn verify_update(request: &VerifyAndCommitRequest, current: Option<&TrustedState>) -> Result<VerifiedUpdate, &'static str> {
    ...
    verify_threshold("risk-decision", &decision, root_keys, role(&root, "risk-decision")?, &revoked, "BLOCK_SIGNATURE_THRESHOLD")?;
    let revoked_decisions: BTreeSet<&str> = revocations.get("revoked_decision_ids").and_then(Value::as_array)
        .ok_or("BLOCK_NATIVE_TRUST_VERIFICATION")?.iter().filter_map(Value::as_str).collect();
    let revoked_epochs: BTreeSet<u64> = revocations.get("revoked_epochs").and_then(Value::as_array)
        .ok_or("BLOCK_NATIVE_TRUST_VERIFICATION")?.iter().filter_map(Value::as_u64).collect();
    if revoked_decisions.contains(string(&decision, "decision_id")?)
        || decision.get("revocation_epoch").and_then(Value::as_u64).is_some_and(|epoch| revoked_epochs.contains(&epoch))
    ...
    for subject in subjects.values() {
        ...
        if !is_sha256(string(subject, "sha256")?)
            ...
            || subject.get("size").and_then(Value::as_u64).is_none_or(|size| size == 0)
        { return Err("BLOCK_NATIVE_TRUST_VERIFICATION"); }
    }
    check_time(&release, "created_at_utc", true)?;
    verify_threshold("release-subjects", &release, root_keys, role(&root, "release")?, &revoked, "BLOCK_SIGNATURE_THRESHOLD")?;
